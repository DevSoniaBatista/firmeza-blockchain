...
contract FirmezaTokenv7b is ERC1155, ReentrancyGuard, Ownable {
    // Estado do contrato
    ...
    function mintProperty(string memory _metadataURI, uint256 _propertyValue, address _seller) public onlyOwner {
    ...
    function setRentDatesAndRenter(uint256 _propertyId, uint256 _startDate, address _renter) public onlyOwner {
        _validatePropertyExists(_propertyId);
    ...
    function investInProperty(uint256 _propertyId, address _investor, uint256 _tokensToBuy) public {
        _validatePropertyExists(_propertyId);
    ...
    function calculateTokenCost(uint256 _propertyId) public view returns (uint256) {
        FirmezaTokenLib.Property storage property = properties[_propertyId];
    ...
    function setPercentageRent(string memory _percentageRent) public onlyOwner {
        uint256 percent = stringToUint(_percentageRent);
        percentageRent = percent;
    }

    function setInitialValueRent(uint256 _propertyId, uint256 _valueRent) public onlyOwner {
        _validatePropertyExists(_propertyId);
        FirmezaTokenLib.Rent storage rent = rents[_propertyId];
