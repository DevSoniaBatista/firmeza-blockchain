...
contract FirmezaTokenv7c is IFirmezaToken, ERC1155, ReentrancyGuard, Ownable {
    // Estado do contrato
    uint256 private propertyIdCounter; // Contador para IDs de propriedades
    string internal baseURI = "https://ipfs.io/ipfs/";
    uint256 internal percentageRent = 5; // Representado em base 1000 para 0.5%
    uint256 internal adminFeeRent = 10; // 10.0% representado em base 1000
    ...
    constructor() ERC1155("FIRMEZA TOKEN") Ownable(msg.sender) {
        propertyIdCounter = 1;
    }

    function mintProperty(string memory _metadataURI, uint256 _propertyValue, address _seller) external restricted {
    ...
    function setRentDatesAndRenter(uint256 _propertyId, uint256 _startDate, address _renter) external restricted {
        _validatePropertyExists(_propertyId);
    ...
    function setRentAdjustment(uint256 _propertyId, string memory _percentAdjustment) external restricted {
        _validatePropertyExists(_propertyId);
    ...
    function receiveRentalPayment(uint256 _propertyId, uint256 _amount) external nonReentrant restricted {
    ...
    function purchaseTokens(uint256 _propertyId, uint256 _tokensToPurchase, uint256 _purchaseDate) external nonReentrant restricted {
        _validatePropertyExists(_propertyId);
    ...
    function setPercentageRent(string memory _percentageRent) external restricted {
        uint256 percent = stringToUint(_percentageRent);
        percentageRent = percent;
    }

    function setInitialValueRent(uint256 _propertyId, uint256 _valueRent) external restricted {
        _validatePropertyExists(_propertyId);
        FirmezaTokenLib.Rent storage rent = rents[_propertyId];
